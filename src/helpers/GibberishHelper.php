...
namespace delaneymethod\spamshield\helpers;

class GibberishHelper
{
    /** @var array */
    public static array $shortWordWhitelist = [
        // 1 letter
        'a', 'i',
        // 2 letters
        ...
        'now', 'off', 'old', 'one', 'our', 'out', 'put', 'see', 'set', 'she', 'the', 'too', 'two', 'use', 'war', 'was', 'way', 'who', 'why', 'you', 'cat', 'dog',
        // 4 letters
    ...
    /**
     * @param string $value
     * @param int $gibberishWordMinimumLength
     * @param array $combinedAllowedTerms
     * @return array
     */
    public static function analyzeGibberish(string $value, int $gibberishWordMinimumLength = 6, array $combinedAllowedTerms = []): array
    {
        ...
        if (!empty($letters) && mb_strlen($letters) <= 4 && !self::isAllowedTerm($letters, $combinedAllowedTerms)) {
        ...
        foreach ($words as $word) {
            ...
            if ($length >= 2 && $length <= 4) {
                ...
                if (!self::isAllowedTerm($letters, $combinedAllowedTerms)) {
                    $hasVowel = (bool) preg_match('/[aeiou]/i', $letters);
            ...
            if ($length < $gibberishWordMinimumLength) {
                continue;
            }
     ...
     * @return bool
     */
    public static function isAllowedTerm(string $value, array $combinedAllowedTerms): bool
    {
